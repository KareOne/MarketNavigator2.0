...
Wraps the Twitter Worker Agent via Orchestrator.
...
class TwitterScraperClient(RetryableScraperClient):
    """
    High-scale client for Twitter scraper API via Orchestrator.
    """
    
    def __init__(self):
        # Base URL for local service (if ever needed fallback)
        base_url = getattr(settings, 'TWITTER_SCRAPER_URL', 'http://twitter_api:8004')
        ...
        self.use_orchestrator = getattr(settings, 'USE_ORCHESTRATOR', True) # Default to True for Twitter
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if total > 0:
                        ...
                        logger.warning("No Twitter workers connected")
    ...
    async def _submit_to_orchestrator(
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(timeout, connect=30.0)) as client:
            ...
            logger.info(f"📤 Twitter task submitted: {task_id}")
            ...
            while elapsed < timeout:
                ...
                if task_status == "completed":
                    logger.info(f"✅ Twitter task completed: {task_id}")
                    return status_data.get("result", {})
                elif task_status in ("failed", "cancelled"):
                    error_msg = status_data.get("error", "Unknown error")
                    raise Exception(f"Twitter task {task_status}: {error_msg}")
            
            raise Exception(f"Twitter task timed out after {timeout}s")
    ...
    async def search_tweets(
        ...
        """
        logger.info(f"Twitter search: {len(keywords)} keywords")
        ...
        if not await self._check_orchestrator_available():
             raise ExternalAPIError("Orchestrator unavailable for Twitter search")
        ...
        try:
            ...
            raise ExternalAPIError(f"Twitter search failed: {e}")
    ...
    async def get_tweet_replies(
        ...
        try:
            ...
            logger.error(f"Twitter reply fetch failed: {e}")
            raise ExternalAPIError(f"Twitter reply fetch failed: {e}")


# Singleton instance
twitter_scraper = TwitterScraperClient()
